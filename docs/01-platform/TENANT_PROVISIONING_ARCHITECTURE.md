...
| Audit | `AuditLog` supports actor, organisation, resource, outcome, timestamp, sanitised metadata, provisioning-operation correlation, provisioning/delivery events, and first-login lifecycle/session evidence. | Recovery event implementation remains deferred. |
| Provisioning | `pnpm tenant:provision` provides strict canonical input, dry-run planning, deterministic idempotency, conflict refusal, durable approval, transactional writes, a fake/test delivery adapter, safe receipts, secret-free output, and a complete restricted first-login activation path. Production execution is deliberately unavailable without a real approved adapter. | Real transactional email, Production execution, recovery commands, and smoke-test automation remain deferred. |
| Password | Argon2id; 12-to-128-character first-login policy; confirmation; common, obvious, context-derived, and temporary-credential reuse rejection; generic invalid-login response; forced first-login rotation. | Privacy-preserving compromised-password checks, rate limiting/lockout, recovery reissue, and general reset remain deferred. |
