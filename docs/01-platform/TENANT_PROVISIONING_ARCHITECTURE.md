...
| --- | --- | --- |
| Organisation | Unique slug, `INSTALLER`/`CLADA_INTERNAL`, `PROVISIONING`/`ACTIVE`/`INACTIVE`/`ARCHIVED`, and `verified`. Provisioning creates installer tenants as unverified `PROVISIONING`; successful first login atomically changes lifecycle status to `ACTIVE` without changing the separate verification flag. PR #30 uses the approved existing `INACTIVE` state for operator suspension and reactivation, preserves all records, and revokes affected sessions. | A dedicated `SUSPENDED` enum remains deferred. |
| User | Unique normalised email, display name, Argon2id hash, `INVITED`/`ACTIVE`/`INACTIVE`, `mustChangePassword`, credential expiry, and last login. Provisioning creates the owner as `INVITED` with a fixed 24-hour temporary-credential expiry; first login enforces expiry and forced replacement before activation. PR #30 adds approved operator suspension/reactivation and credential reissue without exposing secrets. | General password reset, MFA, and owner replacement remain deferred. |
...
| Session | Opaque random token, HMAC-SHA-256 digest in `AuthSession`, explicit normal/restricted type, 30-minute non-refreshable restricted expiry, 12-hour normal expiry, server-side context checks, logout deletion, and all-session rotation after first login. | Recovery-session rules and any future refresh mechanism remain deferred. |
| Audit | `AuditLog` supports actor, organisation, resource, outcome, timestamp, sanitised metadata, provisioning-operation correlation, provisioning/delivery events, first-login lifecycle/session evidence, and PR #30 recovery inspection, reissue, suspension, refusal, and reactivation evidence. | Provider-specific delivery telemetry and GDPR retention remain deferred. |
| Provisioning | `pnpm tenant:provision` provides strict canonical input, dry-run planning, deterministic idempotency, conflict refusal, durable approval, transactional writes, a fake/test delivery adapter, safe receipts, secret-free output, and a complete restricted first-login activation path. PR #30 adds the dry-run-first `pnpm tenant:recover` operator boundary for inspection, credential reissue, suspension, and reactivation. Production execution is deliberately unavailable without a real approved adapter. | Real transactional email, Production execution, owner replacement, and smoke-test automation remain deferred. |
| Password | Argon2id; 12-to-128-character first-login policy; confirmation; common, obvious, context-derived, and temporary-credential reuse rejection; generic invalid-login response; forced first-login rotation; operator credential reissue stores only a replacement Argon2id hash and never returns the temporary credential. | Privacy-preserving compromised-password checks, rate limiting/lockout, and general reset remain deferred. |

The legacy `pnpm pilot:provision` command can still update existing pilot records and accepts plaintext input through an environment variable. It is retained for compatibility but is not the standard tenant-provisioning boundary and must not be used for a new external pilot. The new commands never accept plaintext credentials from input or arguments. Recovery mutations require an active Clada internal approver, an idempotency key, a non-secret reason, and explicit `--execute`; dry-run is the default. Ambiguous lifecycle drift is refused as `MANUAL_REVIEW_REQUIRED`, and owner replacement remains inspection-only.
...
Implemented for first login: a 128-character maximum, common/obvious/context-derived screening, temporary-credential expiry, forced replacement with confirmation and current-credential verification, the dedicated password-change route, and all-session rotation/invalidation. Not currently implemented: privacy-preserving breached-password screening, login rate limiting/lockout, recovery reissue, or general password reset.
