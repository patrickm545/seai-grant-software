...
ADR-0022 defines `pnpm tenant:recover:production-credential` for the incident-bound recovery of one eligible active legacy Production owner. It is read-only by default, requires a reviewed plan and exact Production acknowledgement, accepts a temporary credential only through a hidden interactive prompt, revokes sessions, and forces the existing restricted first-login replacement flow. It does not provision or alter a tenant. The deployed command remains unauthorised for execution until the owner approves a fresh dry-run plan and Production change record; follow the [Production legacy credential reissue runbook](PRODUCTION_LEGACY_CREDENTIAL_REISSUE_RUNBOOK.md).
...
Self-service password reset is now a required pre-pilot capability specified in [FEAT-PRE-PILOT-AUTH-001](../04-features/FEAT-PRE-PILOT-SELF-SERVICE-PASSWORD-RESET.md) and [ADR-0023](../05-decisions/ADR-0023-self-service-password-reset-security-boundary.md). It is not implemented in the current baseline. It will be the normal forgotten-password path; ADR-0022 remains exceptional operational recovery.
...
The current baseline has no self-registration, one-time invitations, general password reset, MFA, SSO, organisation switching, user-management UI, fine-grained custom roles, or session-management UI. General password reset is a planned pilot blocker rather than a permissible pilot limitation. Successful first-login replacement invalidates every existing user session and creates one rotated normal session. Recovery inspection, credential reissue, suspension, and approved reactivation are available only to Clada internal operators through the CLI; owner replacement, real email delivery, GDPR workflows, Production execution, and external pilot onboarding remain deferred.
