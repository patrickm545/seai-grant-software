...
| TD-013 | High | Deployment readiness | Public intake can fail if the hosted database has not received required migrations before updated application code handles traffic. | A correctly built application can still fail at runtime during lead creation if deployment and migration status are not verified together. | Add a release gate or runbook step that checks `prisma migrate status` against the target database before promotion, and include a post-migration smoke submission on preview before production release. | Yes for this hotfix release review. |
| TD-014 | Medium | Notifications | Email and SMS delivery are attempted from the intake request path without a durable notification outbox or retry worker. The hotfix makes them non-blocking for the homeowner response. | Notification provider outages no longer block submissions, but failed notifications require log review and may not retry automatically. | Add a durable notification outbox, retry policy, and admin-visible notification status before commercial scale. | No. |
