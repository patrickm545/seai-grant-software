...
| PCG-004 | Self-service password reset | Required before first external installer pilot | Specified in FEAT-PRE-PILOT-AUTH-001, ADR-0023, the threat model, test plan, and implementation plan; implementation has not begun. | This is a planned security capability, not optional debt. Its absence blocks first-pilot readiness until implementation, approved email delivery, Preview security acceptance, and Production verification are complete. |
