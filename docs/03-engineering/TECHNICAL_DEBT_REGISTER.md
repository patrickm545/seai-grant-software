...
| TD-019 | Critical | Migration provenance | Production contains completed migration `20260423093000_application_pack_admin_fields`, but its repository artifact and checksum-identical SQL are unavailable. Root cause remains G - Unknown. PR #43 implements the deterministic manifest, exact ledger verifier, schema fingerprint and inactive attestation boundary without changing Production. PR #44 prepares fixed read-only evidence-capture tooling and deterministic disposable fresh/post evidence only. Draft PR #45 stopped before Production access because its genuine operational prerequisites and human approvals were unavailable. | The implementation can fail closed independently, but the checked-in attestation remains pending because no live Production evidence, exact related-record IDs, failed-log digest, pre-migration fingerprint or genuine human approvals have been retained. Raw Prisma history remains divergent and release promotion remains blocked. | Resume Draft PR #45 only after its approved change ID, distinct named operator and reviewer, current restore-point reference, controlled Production identity inputs and genuine approvals are available. Then use a different explicitly approved execution PR for the password-reset migration. Retain immutable inventory and deletion/rename controls. | Yes. Blocks every Production release, password-reset request-flow work and pilot rollout until activation, reconciliation and Production verification complete. |
...
until the attestation is independently evidenced, genuinely approved,
