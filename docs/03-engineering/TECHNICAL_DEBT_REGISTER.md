...
| TD-015 | High | Deployment environments | Vercel Preview, Production, Development, and local development currently use the same Neon-hosted PostgreSQL database. Preview testing therefore reads and writes production data and can apply migrations that affect production runtime paths. | Preview submissions and automated testing may create records in the production database. Preview defects, test data, migration experiments, or destructive operations could affect live customer data. Environment-specific verification is also less reliable because Preview is not isolated from Production. | Provision separate Neon databases or isolated Neon branches for Production, Preview, and local development. Configure Vercel environment variables so each environment uses its intended database. Add explicit database fingerprint and ownership checks before migrations. Document promotion, migration, backup, reset, and test-data rules for every environment. | No for Platform Release 1.4 planning, but resolve before commercial-scale production usage or any destructive preview testing. |
