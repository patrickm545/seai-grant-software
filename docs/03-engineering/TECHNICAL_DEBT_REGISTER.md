...
| TD-013 | High | Deployment readiness | Public intake can fail if the hosted database has not received required migrations before updated application code handles traffic. PR #10 amended the shared hosted database with approved Platform Release 1.1-1.3 migrations, but the release process still lacks an automatic migration gate. | A correctly built application can still fail at runtime during lead creation if deployment and migration status are not verified together. | Add a release gate or runbook step that checks `prisma migrate status` against the target database before promotion, and include a post-migration smoke submission on preview before production release. | No after PR #10 amendment; keep as a release-process blocker for future database-backed releases. |
...
| TD-015 | High | Environment isolation | Vercel Preview, Production, Development, and local development currently use the same Neon-hosted database. Preview testing therefore writes test leads and migration changes into the production data store. | Test data can contaminate production, preview defects can affect live data, and database migrations cannot be validated safely in an isolated environment before production. | Provision separate Neon databases or branches for Production, Preview, and Development; scope Vercel environment variables separately; add explicit target identification and migration checks before deployment; and verify that preview tests cannot write to production. | No for tomorrow's demonstration; resolve before regular team testing or commercial production use. |
| TD-016 | Medium | Privacy and communications | SolarGRANT Pro currently sends SMS notifications containing or referencing lead information to installer phone numbers. This can distribute customer information onto personal or unmanaged devices and makes retention, deletion, access control, and offboarding harder to manage. | Customer data may remain on individual phones outside the centrally managed company communication record, increasing GDPR, device-loss, and staff-transition risk. | Remove SMS notifications and related provider configuration from the intake flow in a focused follow-up PR. Use centrally managed email as the supported channel until a governed notification platform with retention, access control, and audit requirements is approved. | No for tomorrow's demonstration; schedule immediately after the hotfix release. |
