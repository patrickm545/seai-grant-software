...
| TD-017 | Low | Product navigation | The authenticated installer navigation currently exposes a Sales Playbook page that is not part of the core pilot installer workflow. | The extra route adds visual clutter and may distract pilot users from daily lead, quote, project, document, and support tasks. | Remove Sales Playbook from authenticated navigation and retire the route unless a validated installer use case is established later. Reassess the full authenticated navigation during pilot UX cleanup. | No. |
| TD-018 | Low | Login usability and accessibility | The login password field does not currently provide a show/hide password control. | Users cannot verify what they typed, increasing avoidable login errors, especially on mobile devices. | Add an accessible show/hide password toggle with clear labelling, keyboard support, and preserved password-manager behaviour. | No. |
