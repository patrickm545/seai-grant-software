...
| TD-015 | High | Deployment environments | **Blocked on provider configuration:** repository-side environment classification, positive Production fingerprint checks, guarded commands, disposable-test checks, CI validation, migration/recovery/smoke runbooks, and runtime Preview/Production mismatch rejection are implemented. Actual Vercel scopes, separate Neon branches, branch protection, retention, and recovery rehearsal remain manually unverified. Authenticated dashboard reads also still perform default identity upserts. | Code now fails closed for named sensitive paths, but isolation is not real until Vercel/Neon mappings are changed and evidenced. Implicit setup writes prevent authenticated Production smoke reads. | Patrick or the deployment owner must complete and evidence the provider checklist, then the authentication/onboarding PR must remove read-time bootstrap upserts. Do not run destructive Preview/integration tests before that work. | Yes for real pilot authentication, onboarding, stored proposals, or destructive integration testing. |
