...
| TD-019 | Critical | Migration provenance | Production contains completed migration `20260423093000_application_pack_admin_fields`, but its repository artifact and checksum-identical SQL are unavailable. Root cause remains G - Unknown. PR #43 implements the deterministic manifest, exact ledger verifier, schema fingerprint and inactive attestation boundary without changing Production. PR #44 adds a fixed read-only pending-evidence command and deterministic disposable fresh/post evidence. | The implementation can fail closed independently, but the checked-in attestation remains pending because exact Production related-record IDs, failed-log digest, pre-migration fingerprint and genuine human approvals have not been recorded. Raw Prisma history remains divergent and release promotion remains blocked. | Complete the separately approved read-only Production evidence capture with a distinct reviewer and restore-point evidence, activate only after genuine approvals, then execute the password-reset migration under a different approved change. Retain immutable inventory and deletion/rename controls. | Yes. Blocks every Production release, password-reset request-flow work and pilot rollout until activation, reconciliation and Production verification complete. |
