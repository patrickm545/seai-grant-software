...
| TD-007 | Medium | Data access coverage | The lead stage-change proving slice now uses a permission-aware service boundary, but some direct Prisma access remains in public-token, seed, setup, and legacy read contexts. | Direct access is acceptable in documented exceptions, but future protected mutations could bypass tenant or permission checks if not reviewed. | Keep public-token and setup exceptions explicit, and move each future protected workflow behind platform service helpers as it is touched. | No. |
