...
| TD-019 | Critical | Migration provenance | Production contains completed migration `20260423093000_application_pack_admin_fields`, but its repository artifact and checksum-identical SQL are unavailable. Root cause remains G - Unknown. PR #43 implements the deterministic manifest, exact ledger verifier, schema fingerprint and inactive attestation boundary without changing Production. | The implementation can fail closed independently, but the checked-in attestation remains pending because exact remaining Production evidence and human approvals have not been recorded. Raw Prisma history remains divergent and release promotion remains blocked. | Independently review PR #43; separately execute the read-only evidence capture and attestation activation; then execute the approved reconciliation and password-reset migration tasks under distinct changes. Retain immutable inventory and deletion/rename controls. | Yes. Blocks every Production release, password-reset request-flow work and pilot rollout until activation, reconciliation and Production verification complete. |
