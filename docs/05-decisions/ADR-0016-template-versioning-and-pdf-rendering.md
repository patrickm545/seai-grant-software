...
Exact byte-for-byte regeneration remains subject to the same interpreter, renderer, fonts, assets, locale, timezone, and PDF metadata controls being available. The stored immutable PDF remains the record. Any later regeneration creates a new immutable `Document` unless the original idempotency identity returns the same existing attempt.
...
- Document Generation, Persistence And Storage must define how the template interpreter and renderer adapter are invoked, how generated PDFs are persisted, and how output checksums are calculated.
