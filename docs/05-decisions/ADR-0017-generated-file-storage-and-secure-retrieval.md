...
| Storage key | Platform-generated opaque unique key, immutable and never an authorisation credential or public identifier. |
...
| Future storage | A later approved adapter may move physical storage after checksum verification without changing product adapters or document content identity. |
...
Storage keys are generated by the platform, preferably from a cryptographically strong random UUID or equivalent opaque identifier with a non-sensitive provider namespace. They are unique, immutable for the stored object, and never reused after a completed write.
...
- separate storage provider and opaque key attribution;
...
| Physical storage location | Provider and opaque key used to retrieve those same bytes. | Yes, through an approved verified migration. |

A later migration must copy bytes, verify size and SHA-256 at the target, preserve prior provider/key attribution in migration audit evidence, atomically change the active physical locator, and retain the same document ID and generated checksum. It must not regenerate or rewrite the PDF. Detailed migration tooling, replication and multi-region design are deferred.
...
| Metadata divergence | Integrity ambiguity. | Require generated-file and `Document` size/checksum equality before `STORED`. |
...
No implementation is authorised by this ADR while it remains `Proposed`.
...
1. Complete final CTO review and either accept or revise ADR-0017; it remains `Proposed` until then.
2. Treat this as the final planned architecture ADR before the pilot implementation sprint unless implementation exposes a genuine material blocker.
3. Complete the four Release 1.4 feature specifications.
4. Complete generation transactions, idempotency and failure-handling work at implementation-specification level.
5. Resolve TD-015 before hosted migrations or generated-document storage tests.
6. Begin implementation only after explicit approval; do not create a release tag as part of this ADR work.
