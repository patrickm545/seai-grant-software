...
The PR 2 draft branch implements `/installer-review-emerald/leads/new` for roles with `lead.create`, but the route, entry actions, server action, duplicate lookup and service are all closed unless the typed environment gate receives the exact explicit enabling value. It accepts name plus phone/email and optional address, Eircode, approved source, follow-up, membership assignee, and append-only internal note; creates the existing lead/workflow/activity/audit records atomically; and redirects to the canonical workspace. Manual records have explicit origin and trusted creator attribution, organisation-scoped request-token replay, no portal invitation or messaging, and null qualification/consent facts. The branch is pending review/merge; Production and Preview remain disabled pending Project Shield and company/privacy-owner approval.
